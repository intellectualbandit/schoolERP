...
import {
  ...
  TrendingDown, ArrowUpRight, ArrowDownRight, CalendarDays, Bell,
  AlertTriangle, CheckCircle2, Clock, DollarSign, UserCheck, BookOpen,
  Activity, ChevronRight, BarChart3, PieChart as PieIcon, Megaphone,
  Star, Award, Target, Zap, Eye, FileText, ShieldAlert, Heart
} from 'lucide-react';
...
function StatCard({ label, value, icon: Icon, color, bgColor, change, changeType, subtitle }) {
  return (
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data}>
        ...
        <Area type="monotone" dataKey={dataKey} stroke={color} fill={`url(#spark-${dataKey})`} strokeWidth={2} dot={false} />
      </AreaChart>
    </ResponsiveContainer>
  );
}
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Welcome back, Admin. Here's your school overview for <span className="font-medium text-foreground">March 24, 2026</span>
          </p>
        </div>
      ...
      {/* ─── Row 2: Enrollment Trend + Demographics ─── */}
                    ...
                    <div key={g.name} className="space-y-1">
                      ...
                      <div className="h-2 rounded-full bg-gray-100 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(g.value / 325) * 100}%`, backgroundColor: g.fill }}
                        />
                      </div>
                    </div>
                  ))}
      ...
      {/* ─── Row 3: Attendance + Fee Collection ─── */}
      ...
      {/* ─── Row 4: Academic Performance + Department Performance ─── */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                  ...
                  <tr className="border-b">
                    ...
                    <th className="text-center text-xs font-medium text-muted-foreground py-2.5 px-3">
                      <span className="inline-flex items-center gap-1">
                        <span className="h-2 w-2 rounded-full bg-emerald-500" />Outstanding
                      </span>
                    </th>
                    <th className="text-center text-xs font-medium text-muted-foreground py-2.5 px-3">
                      <span className="inline-flex items-center gap-1">
                        <span className="h-2 w-2 rounded-full bg-blue-500" />Satisfactory
                      </span>
                    </th>
                    <th className="text-center text-xs font-medium text-muted-foreground py-2.5 px-3">
                      <span className="inline-flex items-center gap-1">
                        <span className="h-2 w-2 rounded-full bg-red-500" />Did Not Meet
                      </span>
                    </th>
                    ...
                    return (
                      <tr key={g.grade} className="border-b last:border-0 hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="py-3 px-3 text-center">
                          <span className="inline-flex items-center justify-center h-7 w-12 rounded-md bg-emerald-50 text-emerald-700 text-xs font-semibold">
                            {((g.outstanding / total) * 100).toFixed(0)}%
                          </span>
                        </td>
                        <td className="py-3 px-3 text-center">
                          <span className="inline-flex items-center justify-center h-7 w-12 rounded-md bg-blue-50 text-blue-700 text-xs font-semibold">
                            {((g.satisfactory / total) * 100).toFixed(0)}%
                          </span>
                        </td>
                        <td className="py-3 px-3 text-center">
                          <span className="inline-flex items-center justify-center h-7 w-12 rounded-md bg-red-50 text-red-700 text-xs font-semibold">
                            {((g.didNotMeet / total) * 100).toFixed(0)}%
                          </span>
                        </td>
                        <td className="py-3 pl-3 text-center">
                          <span className="text-sm font-bold">{g.average}</span>
                        </td>
      ...
      {/* ─── Row 5: Department Performance + Activity Feed + Events ─── */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
